...
This directory contains Kubernetes manifests for deploying {{cookiecutter.app_name}}.
...
```bash
# Edit secrets
kubectl edit secret {{cookiecutter.app_name}}-secrets -n {{cookiecutter.app_name}}

# Or create from literal
kubectl create secret generic {{cookiecutter.app_name}}-secrets \
...
annotations:
  ...
  prometheus.io/port: "{{cookiecutter.server_port}}"
...
```bash
kubectl scale deployment {{cookiecutter.app_name}} --replicas=5 -n {{cookiecutter.app_name}}
```
...
```bash
# Check pod status
kubectl get pods -n {{cookiecutter.app_name}}

# View logs
kubectl logs -f deployment/{{cookiecutter.app_name}} -n {{cookiecutter.app_name}}

# Describe deployment
kubectl describe deployment {{cookiecutter.app_name}} -n {{cookiecutter.app_name}}

# Check events
kubectl get events -n {{cookiecutter.app_name}} --sort-by='.lastTimestamp'

# Port forward for local testing
kubectl port-forward svc/{{cookiecutter.app_name}} 8080:80 -n {{cookiecutter.app_name}}
```
