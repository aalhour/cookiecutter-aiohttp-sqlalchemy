...
from example_web_app.tasks.jobs import example_task, send_notification
...
class TestEnqueue:
    ...
    @pytest.mark.asyncio
    @patch('example_web_app.tasks.worker.get_task_pool')
    async def test_enqueue_returns_job_id(self, mock_get_pool):
        """Test that enqueue returns job ID."""
        from example_web_app.tasks import enqueue
